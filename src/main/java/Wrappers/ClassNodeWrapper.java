...
public class ClassNodeWrapper {
	...
	public ClassNodeWrapper(ClassNode classNode) {
		...
		this.supername = classNode.superName == null ? Optional.empty()
		...
		if (classNode.fields != null) {
			for (FieldNode fieldNode : (List<FieldNode>) classNode.fields) {
				this.fieldNodeWrappers
						.add(new FieldNodeWrapper(fieldNode, Type.getType(fieldNode.desc).getClassName()));
				...
				if (methodNode.signature != null) {
					...
					if (Type.getArgumentTypes(methodNode.desc).length != 0) {
						for (int i = 0; i < Type.getArgumentTypes(methodNode.desc).length; i++) {
							populateRelationsWithNonGenerics(this.dependencies,
